...
{
    // Інтерфейс для об'єктів, які можуть запускати ігри
    ...
    public abstract class Device
    {
        ...
        public virtual bool UninstallGame(Game game)
        {
            ...
            // Видалення гри
            int freedSpace = game.Uninstall();
            if (freedSpace > 0)
            {
                InstalledGames.Remove(game);
                FreeHddGB += freedSpace;
                
                if (CurrentlyRunningGame == game)
                {
                    CurrentlyRunningGame = null;
                }
