...
using Lab1_3.UI.Console;
using Lab1_3.UI.Console.Menus;
...
public class GameSimulator
{
    ...
    public GameSimulator(string gamesJsonPath)
    {
        ...
        _deviceMenu = new DeviceMenu(_view, ValidationService, _devices, _eventSystem);
        _gameMenu = new GameMenu(_view, ValidationService, _gameRepository, _eventSystem);
        _accountMenu = new AccountMenu(_view, ValidationService, _accountService, _eventSystem);
    ...
    public void AddDevice()
    {
        ...
        if (device != null && CurrentDevice == null) CurrentDevice = device;
    }
    ...
    public void RunGame()
    {
        ...
        if (CurrentDevice.CurrentlyRunningGame == null)
        {
            ...
            if (installedGames.Count > 0)
            {
                ...
                for (var i = 0; i < installedGames.Count; i++) _view.DisplayInfo($"{i + 1}. {installedGames[i]}");

                var gameIndex = ValidationService.ParseInt(
                ...
                if (selectedGame.IsOnline)
                {
                    if (CurrentAccount == null)
                    {
                        ...
                        var createAccount = ValidationService.ParseBool(
                    ...
                    _view.DisplayInfo(
                        $"Запуск онлайн-гри {selectedGame.Name} з обліковим записом {CurrentAccount!.Username}");
                    ...
                    {
                        _view.DisplayError(
                            $"Не вдалося запустити гру {selectedGame.Name} на пристрої {CurrentDevice.Name}");
                    }
                }
                else
                {
        ...
        {
            _view.DisplayError(
                $"На пристрої {CurrentDevice.Name} вже запущена гра {CurrentDevice.CurrentlyRunningGame.Name}");
            _view.WaitForKeyPress();
        }
    }
