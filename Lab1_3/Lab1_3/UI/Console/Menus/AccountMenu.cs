...
public class AccountMenu
{
    ...
    public AccountMenu(IConsoleView view, ValidationService validationService, AccountService accountService,
    ...
    public void Show()
    {
        ...
        while (!exit)
        {
            ...
            var currentAccount = _accountService.CurrentAccount != null
            ...
            var choice = _validationService.ParseInt(
    ...
    public void ShowAllAccounts()
    {
        ...
        {
            var current = _accountService.CurrentAccount == accounts[i] ? " (поточний)" : "";
    ...
    public void CreateAccount()
    {
        ...
        var username = _view.GetInput("Введіть логін: ");
        if (!_validationService.IsValidName(username))
        {
            _view.DisplayError(
                "Логін повинен містити від 3 до 20 символів і складатися з літер, цифр та символів _ або -");
            _view.WaitForKeyPress();
            return;
        }

        var isPremium = _validationService.ParseBool(_view.GetInput("Бажаєте створити преміум-акаунт? (так/ні): "));
        ...
        if (account != null)
        {
            ...
            var login = _validationService.ParseBool(
                _view.GetInput("Бажаєте увійти в новостворений акаунт? (так/ні): "));
            if (login) _accountService.Login(username);
        }
        else
        {
    ...
    public void Login()
    {
        ...
        if (_accountService.CurrentAccount != null)
        {
            ...
            var logout =
                _validationService.ParseBool(_view.GetInput("Бажаєте вийти з поточного облікового запису? (так/ні): "));
            if (logout)
            {
        ...
        for (var i = 0; i < accounts.Count; i++) _view.DisplayInfo($"{i + 1}. {accounts[i]}");

        var accountIndex = _validationService.ParseInt(
    ...
    public void Logout()
    {
        ...
        var confirm = _validationService.ParseBool(_view.GetInput("Підтвердіть вихід (так/ні): "));
